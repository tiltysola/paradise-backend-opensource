...
export default class {
  ...
  static async run(ctx: Koa.ParameterizedContext<Koa.DefaultState, Koa.DefaultContext>) {
    const { charId, nickname, skinType } = ctx.request.body;
    if (!isString(charId) || !regexp.uuid.test(charId)) {
      ...
      } else if (nickname && character?.getDataValue('nickname') !== nickname && (await characterService.exist({ nickname }))) {
        ctx.body = ctx.code('NICKNAME_EXIST');
      } else {
        await characterService.modify({ uuid: charId }, { nickname, skinType });
